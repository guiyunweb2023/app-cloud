...
import org.example.auth.sdk.jwt.UserAuthUtil;
import org.example.common.constant.RedisKeyConstant;
import org.example.common.context.BaseContextHandler;
import org.example.common.exception.auth.UserTokenException;
import org.example.common.msg.BaseResponse;
import org.example.common.msg.auth.TokenForbiddenResponse;
import org.example.common.util.jwt.IJWTInfo;
import org.example.gateway.handler.RequestBodyRoutePredicateFactory;
import org.example.gateway.service.LogService;
...
public class AccessGatewayFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange serverWebExchange, GatewayFilterChain gatewayFilterChain) {
        ...
        Mono<CheckPermissionInfo> checkPermissionInfoMono = webClientBuilder.build().
                get().uri("http://app-admin/api/user/{username}/check_permission?requestMethod=" + method + "&requestUri=" + requestUri, user.getUniqueName()).header(userAuthConfig.getTokenHeader(), BaseContextHandler.getToken()).retrieve().bodyToMono(CheckPermissionInfo.class);
